ig.module('game.camera').defines(function ()
{
    Camera = ig.Class.extend(
    {
        ...
        draw: function ()
        {
            ...
            if( ig.ua.mobile ) scale=2;
            
                ig.system.context.fillStyle = 'rgba(255,0,255,0.3)';
                ig.system.context.fillRect((this.trap.pos.x - this.pos.x) * scale, (this.trap.pos.y - this.pos.y) * scale, this.trap.size.x * scale, this.trap.size.y * scale);
            }
        }
    });
});